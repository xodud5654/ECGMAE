...
class MaskedAutoencoderViT(nn.Module):
    ...
    def random_masking(self, x, mask_ratio):
        ...
        for i in range(N):
            zero = random.sample(range(0,14),4)
            for j in zero:
                for z in range(14):
                    noise[i][j+14*z] = 0
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        # pred_1 = self.unpatchify(pred)
        # plt.figure()
        # plt.subplot(1,2,1)
        # plt.imshow(imgs[0,:,:].permute(1,2,0).detach().cpu().numpy()*255.,vmin=0, vmax=255)
        # plt.subplot(1,2,2)
        # plt.imshow(pred_1[0,:,:].permute(1,2,0).detach().cpu().numpy()*255.,vmin=0, vmax=255)
        # plt.show()
