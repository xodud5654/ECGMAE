...
def get_args_parser():
    ...
    parser.add_argument('--warmup_epochs', type=int, default=20, metavar='N',                                                                     #revise
...
def main(args):
    ...
    ###############################################################################################################
    import glob2
    # scalo_train = glob2.glob("/media/airl2/hdd1/SSL_ECG/data/grayImg_ALL_lead(whiteline)/train/*/*.png")
    # scalo_valid = glob2.glob("/media/airl2/hdd1/SSL_ECG/data/grayImg_ALL_lead(whiteline)/val/*/*.png")
    
    
    scalo_train = glob2.glob("/media/airl2/hdd1/SSL_ECG/data/gray/*/train/*/*.png")
    scalo_valid = glob2.glob("/media/airl2/hdd1/SSL_ECG/data/gray/*/val/*/*.png")
    
    total_data_path = scalo_train + scalo_valid
    # total_data_path = glob2.glob("/mnt/hdd1/SSL_ECG/data/grayImg_ALL_lead/*.png")
    # total_data_path = glob2.glob("/mnt/hdd1/SSL_WBC/data/data1/Labelled/*/*bmp")
    # total_data_path = glob2.glob("/mnt/hdd1/Capsule/unlabelled_images_every_10frames/*.png")

    # CXR8 = glob2.glob("/mnt/hdd1/SSL_CXR/data/CXR8/images/*/*/*.png")
    # CheXpert = glob2.glob("/mnt/hdd1/SSL_CXR/data/CheXpert-v1.0-small/*/*/*/view1_frontal.jpg")
...
#     for mask in [0.65,0.7,0.75,0.8,0.85,0.9,0.95]:  # 0.65,0.7,0.75,0.8,0.85,0.9,0.95
#         args.mask_ratio = mask
#         args.output_dir = f"/mnt/hdd1/Capsule/result/pretrain/Capsule_{args.epochs}epoch_{args.batch_size}batch_{args.blr}lr_{args.mask_ratio}ratio"
...
if __name__ == '__main__':
    args = get_args_parser()
    args = args.parse_args()
    # for mask in [0.65,0.7,0.75,0.8,0.85,0.9,0.95]: 
    
    for mask in [0.71]: 
        args.mask_ratio = mask
        args.output_dir = f"/mnt/hdd1/SSL_ECG/result/pretrain/MAE/vertical/Gray_lead_{args.epochs}epoch_{args.batch_size}batch_{args.blr}lr_{args.mask_ratio}ratio"
        args.log_dir = args.output_dir
        if args.output_dir:
            Path(args.output_dir).mkdir(parents=True, exist_ok=True)
        main(args)
