...
if __name__ == '__main__':  
    for lr in [1e-3]:   
        i = 42
        for epoch in [500]:
            for lead in range(1,13,1):
                train_path = f"{data_path}/lead_{lead}/txt"
                for m, f, n in [
                                
                                ["vit_small_patch16",f"{model_path}/Gray_lead/checkpoint-{epoch-1}.pth","sigMAE"],
                                ["BYOL", f"{model_path}/BYOL/model_Epoch_{epoch-1}.pth", "BYOL"],
                                ["moco",f"{model_path}/moco/checkpoint_0{epoch-1}.pth","MOCO"]
                                ]:
                    ...
                    args.seed           = i
                    args.train_list     = f"{train_path}/train.txt"
                    args.val_list       = f"{train_path}/val.txt"
                    args.test_list      = f"{train_path}/test.txt"
                    args.output_dir     = f"{path}/{n}/{lr}/lead_{lead}/{epoch}E/{args.seed}"
